...
elif jeu == "Fortnite": st.subheader("Stats Fortnite") victoires = st.number_input("Victoires", 0) parties_jouees = st.number_input("Parties jouées", 1) tirs_touches = st.number_input("Tirs touchés", 0) tirs_rates = st.number_input("Tirs ratés", 0) eliminations = st.number_input("Eliminations", 0)
...
if st.button("Analyse ton shoot"):
    ...
    st.write(f"Eliminations totales : {eliminations}")
    ...
    if precision > 0.4:
        ...
        conseil("Améliore ta visée pour fair
    e plus de dégâts.")
